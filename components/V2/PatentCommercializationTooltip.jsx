...
const PatentCommercializationTooltip = ({ children, assetId, tileIndex, totalTiles, isEnabled = true }) => {
  const tooltipRef = useRef(null);
  const [tooltipPosition, setTooltipPosition] = useState('center');
  
  const formData = useV2Store((state) => state.getFormData('patentCommercialization'));
  ...
  const getTooltipData = () => {
    return {
      stage: formData.pcPatentCommercializationChildStage || 'Empty',
      workingFiled: formData.pcPatentCommercializationChildWorkingFiled || 'Empty',
      firstWorkingDate: formData.pcPatentCommercializationChildFirstWorkingDate || 'Empty',
      patentApplicationNumber: formData.pcPANPatentApplicationNumber || 'Empty',
      productId: formData.pcPatentCommercializationEffortsProductId || 'Empty'
    };
  };
        ...
        >
          ...
          <div className="space-y-2">
            <div className="font-semibold text-blue-300 border-b border-gray-700 pb-1 mb-2">
              Patent Commercialization Details
            </div>
            
            <div className="space-y-1">
              <div className="flex justify-between">
                <span className="text-gray-300">Stage:</span>
                <span className="text-white font-medium">{tooltipData.stage}</span>
              </div>
              
              <div className="flex justify-between">
                <span className="text-gray-300">Working Filed:</span>
                <span className="text-white">{tooltipData.workingFiled}</span>
              </div>
              
              <div className="flex justify-between">
                <span className="text-gray-300">First Working Date:</span>
                <span className="text-white">{tooltipData.firstWorkingDate}</span>
              </div>
              ...
              <div className="flex justify-between">
                ...
                <span className="text-white">{tooltipData.productId}</span>
              </div>
            </div>
          </div>
        </div>
...
export default PatentCommercializationTooltip;
