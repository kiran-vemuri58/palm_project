...
const PatentFilingTooltip = ({ children, assetId, tileIndex, totalTiles, isEnabled = true }) => {
  const tooltipRef = useRef(null);
  const [tooltipPosition, setTooltipPosition] = useState('center');
  
  const formData = useV2Store((state) => state.getFormData('patentFiling'));
  ...
  const getTooltipData = () => {
    return {
      ...
      averageRating: formData.averageRating || 'Empty',
      patentabilityRating: formData.patentabilityRating || 'Empty',
      extractorOne: formData.extractorOne || 'Empty',
      hoursSpent: formData.hoursSpent || 'Empty'
    };
  };
          ...
          <div className="space-y-2">
            ...
            <div className="space-y-1">
              ...
              <div className="flex justify-between">
                ...
                <span className="text-white">{tooltipData.averageRating}</span>
              </div>
              
              <div className="flex justify-between">
                <span className="text-gray-300">Patentability Rating:</span>
                <span className="text-white">{tooltipData.patentabilityRating}</span>
              </div>
              ...
              <div className="flex justify-between">
                ...
                <span className="text-white">{tooltipData.hoursSpent}</span>
              </div>
            </div>
          </div>
        </div>
...
export default PatentFilingTooltip;
